...
/*   Updated: 2024/03/24 11:00:19 by yassine          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	*remaining = ft_substr2(a.buffer, a.size_line, a.readn - a.size_line, 0);
	if (!*remaining)
...
char	*ft_newline(char **remaining, struct s_vbs a, int pos)
{
	...
	*remaining = ft_substr2(*remaining, pos + 1, ft_strlen2(*remaining) - pos - 1,
			1);
	if (!*remaining)
		return (free(*remaining), free(a.line), NULL);
	return (a.line);
...
char	*get_next_line(int fd)
{
	...
	if (remaining)
	{
		if (ft_strchr2(remaining, '\n') != -1)
			return (ft_newline(&remaining, a, ft_strchr2(remaining, '\n')));
		a.line = ft_strjoin2(a.line, remaining, ft_strlen2(remaining), 0);
		remaining = NULL;
		a.str_free = 1;
	}
	a.readn = read(fd, a.buffer, BUFFER_SIZE);
