...
// mlx_init is the first function to call, it returns a void * which is a pointer to a t_xvar structure 
// it is meant to be used as a parameter for all mlx functions 
...
// mlx_loop is a blocking function, it will wait for events and handle them until the last window is closed 
// (or until the application is terminated by the user) 
...
// mlx_pixel_put is a simple function to put a pixel in a window
// it is not mandatory to use it, you can use mlx_put_image_to_window instead
...
// mlx_loop_once is a non-blocking function, it will handle one event and return 
...
// this function is called by the window close button (NSWindowDelegate protocol) 
// and by the application when it terminates
// 